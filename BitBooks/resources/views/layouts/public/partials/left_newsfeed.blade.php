    ...
    <div class="profile-card">
        @foreach($friend as $user)
            @if(isset($user->user_image))
                <img src="{{ $user->user_image }}" class="profile-photo" />
            @else
                <img src="{{ asset('assets/images/man-icon-2x.png') }}" class="profile-photo" />
            @endif
        @endforeach
        <h5><a href="{{ url('Timeline') }}" class="text-white">{{ Auth::user()->name }}</a></h5>
